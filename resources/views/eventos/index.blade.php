...
<div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
    ...
    <div class="col">
        <div class="card mb-3
            {{ $evento->ESTADO == 'inactiva' ? 'border-danger' :
               ($evento->ESTADO == 'activa' ? 'border-success' : 'border-secondary') }}">
            <div class="card-header
                {{ $evento->ESTADO == 'inactiva' ? 'bg-danger text-white' :
                   ($evento->ESTADO == 'activa' ? 'bg-success text-white' : 'bg-light') }}">
                {{ $evento->NOMBRE }}
            </div>
            <div class="card-body">
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        cards.forEach((card, index) => {
            ...
            const detalles = card.querySelector(".card-text")?.innerText.trim(); // innerText para conservar saltos
